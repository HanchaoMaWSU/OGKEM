...
public class DBPediaInduceGraphGenerator2 {
    ...
    public DBPediaInduceGraphGenerator2(QueriesDependencyGraph queryGraph, DBPediaDataGraph dataGraph, DBPediaOntologyGraph oGraph, int alpha) {
    ...
    public void generateSizedGraphs() {
        ...
        for (String queryRoot : queryGraph.getQueries().keySet()) {
            ...
            this.graphMaps.put(currentQuery.getRoot(), iGraph);
            System.out.println(graphMaps.get(queryRoot).vertexSet().size() + " " + graphMaps.get(queryRoot).edgeSet().size());
        }

    }


    private DefaultDirectedGraph<DataNode, RelationshipEdge> getRandomIGraph(OGKQuery ogkQuery, Graph<DataNode, RelationshipEdge> dataGraph) {
        ...
        DefaultDirectedGraph<DataNode, RelationshipEdge> iGraph = generateRandomInducedGraph(ogkQuery, predicateMap, oGraph, dataGraph);

        return iGraph;
    }

    private DefaultDirectedGraph<DataNode, RelationshipEdge> generateRandomInducedGraph(OGKQuery query, HashSet<String> predicateSet, DBPediaOntologyGraph oGraph, Graph<DataNode, RelationshipEdge> dataGraph) {
        ...
        for (DataNode dataNode : dataGraph.vertexSet()) {
            ...
            if (dataNode.getTypes().contains(query.getRoot())) {
    ...
    private boolean checkLiterals(Graph<DataNode, RelationshipEdge> dataGraph, DataNode dataNode, HashMap<String, String> literals) {
