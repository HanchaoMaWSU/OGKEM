...
public class BGKQueryRunner {
    ...
    private DBPediaInduceGraphGenerator2 induceGraphs;
    private double theta;
    private int alpha;
    public static HashSet<OGKQuery> piStar = new HashSet<>();
    public static Stack<StackTriple> entriesStack = new Stack<>();

    public BGKQueryRunner(QueriesDependencyGraph queryGraph, DBPediaDataGraph dataGraph, DBPediaOntologyGraph oGraph,
                             double theta, int alpha, DBPediaInduceGraphGenerator2 induceGraphs) {
    ...
    private void dfsRefine(ArrayList<OGKQuery> openList, ArrayList<OGKQuery> nextList, HashSet<OGKQuery> pi,
        ...
        while (lSet.size() != 0) {
            ...
            System.out.print("nextSize" + lSet.size());
    ...
    private HashSet<OGKQuery> getLSet(ArrayList<OGKQuery> openList, int b, Stack<StackTriple> entriesStack) {
        ...
        for (int i = 0; i < openList.size(); i++){
            ...
            System.out.print("currentUd" + query.ud );
    ...
    private boolean isEmbedding(ArrayList<DataNode> currentMatch, ArrayList<DataNode> patternTypes,
                                double theta, DBPediaOntologyGraph oGraph, int rootIndex, OGKQuery ogkQuery) {
    ...
    private void getInitialMatches(OGKQuery currentQuery) {
        ...
        if (!currentQuery.isC) {
            ...
            for (DataNode dataNode : induceGraph.vertexSet()) {
                ...
                if (similarLabelSrc.contains(currentQuery.getRoot())) {
            ...
            for (DataNode dataNode : induceGraph.vertexSet()) {
                if (similarLabels.contains(currentQuery.getRoot())) {
                    currentQuery.getResultMatches().classSet.put(dataNode, new HashMap<Integer, Integer>());
                }
            }
